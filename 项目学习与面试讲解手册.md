...
## 13. 2026-03-05 增量：用户角色管理（RBAC 关系维护）

### 13.1 新增内容
1. 新增 `UserRoleService` 与 `UserRoleController`。
2. 新增用户角色关系 API（查询/覆盖设置/添加/移除）。
3. 安全策略补充：`/api/users/**` 仅 `ADMIN` 可访问。
4. 登录鉴权链路统一：登录与 `UserDetailsService` 均从用户角色关系表获取角色。

### 13.2 新增接口清单
1. `GET /api/users/{userId}/roles`
2. `PUT /api/users/{userId}/roles`（Body: `List<Long>`）
3. `POST /api/users/{userId}/roles/{roleId}`
4. `DELETE /api/users/{userId}/roles/{roleId}`

### 13.3 面试可讲点（建议）
1. 为什么关联表不直接用 BaseMapper：`sys_user_roles` 为联合主键（`user_id + role_id`），默认单主键模型不匹配。
2. 如何保证权限来源一致：登录签发 JWT 和认证加载权限使用同一套角色查询逻辑，避免口径不一致。
3. 如何设计角色维护接口：覆盖设置（批量）+ 增量调整（单条），兼顾运营效率和审计可读性。

### 13.4 联调建议
1. 管理员页面增加“用户角色管理”菜单，运维账号不展示。
2. 角色覆盖保存后建议目标用户重新登录，确保 token 角色声明与数据库一致。